...
function DoOnNavMenuTransitioned()
{
	...
	let divNavArrow = document.getElementById("navigation_arrow");
	
	if (divNavMenu && divNavArrow)
	{
...
function DoOpenCloseMenu(bDoToggle)
{
	...
	let strMenuText = "◄ <span class=hamburger>≡</span> MENU ◄";
	let bOpen = JSON.parse(sessionStorage.getItem("menu_open"));

	if (spanMenuText && divNav && divNavMenu && divNavArrow && divContent)
	{
		if (bDoToggle)
		{
		...
		if (bOpen)
		{
			...
			divContent.style.width = "calc(var(--body_width) - var(--nav_width) - var(--nav_menu_arrow_width) - var(--nav_menu_open_width_adjust))";
			divContent.style.maxWidth = divContent.style.width;
			//divNavArrow.style.left = "6px";
			//divNavArrow.style.top = "-219px";
		}
		else
		{
			...
			divContent.style.width = "calc(var(--body_width) - var(--nav_menu_arrow_width) - var(--nav_menu_close_width_adjust))";
			divContent.style.maxWidth = divContent.style.width;			
			//divNavArrow.style.left = "0px";
			//divNavArrow.style.top = "0px";
		}
